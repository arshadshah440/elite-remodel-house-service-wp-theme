<?php
/**
 * Location page section: Services teaser.
 *
 * Reuses the exact .erh-service-card component from the home page's "Our
 * Service" section (home.css) - either the hand-picked services chosen in
 * "Choose services to show", or every published service when left empty.
 *
...
$erh_title    = erh_field( 'location_services_title' );

if ( ! $erh_title ) {
	/* translators: %s: location name, e.g. "Austin, TX". */
	$erh_title = sprintf( __( 'Remodeling Services in %s', 'elite-remodel-hub' ), $erh_location );
}

$erh_selected = erh_field( 'location_services_selected' );
...
<section class="erh-services erh-section" id="services">
	<div class="erh-container">
		...
		<div class="erh-section-head">
			<?php erh_split_heading( $erh_title, (int) erh_field( 'location_services_title_highlight' ), 'h2' ); ?>
			<?php $erh_text = erh_field( 'location_services_text' ); ?>
			<?php if ( $erh_text ) : ?>
